...
int main() {
    ...
    // exam total
    if (fExamTotal > ((examTotal + fExamTotal) / (examNumber + 1))) {
        exam_average = fExamTotal;
    }

    else {
        exam_average = (examTotal + fExamTotal) / (examNumber + 1);
    }



    // hw total
    if (hwNumber == 0) {hwNumber++;}
    if (lwNumber == 0) {lwNumber++;}
    hw_average = hwTotal / hwNumber;

    // lw total
    lw_average = (lwTotal / lwNumber) * 100;
    ...
    double weighted_total = (exam_average * .4) + (engagement * .1) + (hw_average * .4) + (lw_average * .1);
